...
class Controller:
    ...
    def trigger(self, utc_offset, utc_sign, alert_uri):
    ...
    def fill_fields(self):
        ...
        with open("./bugme/user_data.json", "r") as data_file:
            try:
                ...
                self.token_input.insert(0, data['user_info'][0]['token'])
                self.frequency_input.insert(0, data['user_info'][0]['frequency'])
                self.offset_amount.insert(0, data['user_info'][0]['utc_offset'])
                
                if(data['user_info'][0]['utc_sign'] == [1]):
                    self.offset_sign.activate(0)
                else:
                    self.offset_sign.activate(1)

                self.alert_uri.insert(0, data['user_info'][0]['alert_uri'])
            except:
                pass

    def grab_user_data(self, token, frequency, utc_offset, utc_sign, alert_uri):
    ...
    def watch(self, token, frequency, utc_offset, utc_sign, alert_uri):
        ...
        while True:
            if datetime.now() > last_time+timedelta(minutes=int(frequency)):
                ...
                self.trigger(utc_offset, utc_sign, alert_uri)
...
if __name__ == "__main__":
    ...
    with open("./bugme/user_data.json") as data_file:
        data = json.load(data_file)
        watch_process = multiprocessing.Process(target=c.watch, args=(
